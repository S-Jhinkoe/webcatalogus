...
function displayProducts() {
    fetch(`${Link}`).then((creditis) => creditis.json())
        .then((productInfos) => {
            let products = productInfos;
            console.log('prodcts: ', products);

            for (let i = 0; i < products.length; i++) {
                products[i];
                const htmlCoding = `                
                <h3 class="object-name">${products.title}</h3>
                <h3 class="object-colour">${products.colour}</h3>
                <h4 class="object-description">${products.description}</h4>
                <h3 class="object-size">${products}</h3>
                <h3 class="price">${products.price} euros</h3>
                <h3>Sizes</h3>
                <div class="size-container sizes1">
                <label class="size-1">${products.size[0]}</label>
                <input type="radio" name="adult" class="adult-2" value="true">
                </div>
                <div class="size-container sizes2">
                <label class="size-2">${products.size[1]}</label>
                <input type="radio" name="adult" class="adult-3" value="false">
                </div>
                <div class="size-container sizes3">
                <label class="size-3">${products.sizes[2]}</label>
                <input type="radio" name="adult" class="adult-4" value="true">
                </div>`;
            }
